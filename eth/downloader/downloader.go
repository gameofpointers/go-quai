...
func New(stateDb ethdb.Database, mux *event.TypeMux, core Core, dropPeer peerDropFn) *Downloader {
	dl := &Downloader{
		stateDB:         stateDb,
		mux:             mux,
		queue:           newQueue(core, blockCacheMaxItems, blockCacheInitialItems),
		peers:           newPeerSet(),
		core:            core,
		dropPeer:        dropPeer,
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	// If the context is prime, there is no skeleton fetch.
	// This branch of code is updating the localHeight based on the different
	// FetchDepths.
	if nodeCtx == common.PRIME_CTX {
		skeleton = false
		d.queue.Prepare(localHeight, FullSync)
	}
	...
	updateFetchPoint := func() {
		if nodeCtx == common.PRIME_CTX {
			if localHeight+uint64(MaxHeaderFetch) < peerHeight {
				// Special case for knot in the case of prime.
				if localHeight <= 9 {
					from = uint64(MaxHeaderFetch)
				} else {
					from = localHeight + uint64(MaxHeaderFetch)
				}
			} else {
				from = peerHeight
			}
	...
	getHeaders := func(from uint64, to uint64) {
		...
		if skeleton {
			...
			go p.peer.RequestHeadersByNumber(from, MaxSkeletonSize, to, true, true)
		} else {
			p.log.Trace("Fetching full headers", "count", MaxHeaderFetch, "from", from)
			go p.peer.RequestHeadersByNumber(from, MaxHeaderFetch, to, false, true)
		}
	}
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			// This is only used for prime and getting the tail for region and zone.
			if len(headers) > 0 {
				...
				if nodeCtx == common.PRIME_CTX || !skeleton {
					localHeight += uint64(len(headers))
				}

				if localHeight >= peerHeight+1 {
					continue
				}
...
// headEventLoop listens to the waitEvent in slice and assigns the value
// to the d.waitingOnAppend.
func (d *Downloader) headEventLoop() {
