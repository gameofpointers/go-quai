	...
	for i := 1; errc == nil && merr == nil; i++ {
		...
		for _, line := range lines {
			...
			for idx, counter := range parts[2:] {
				...
				if err != nil {
					db.logger.Error("Compaction entry parsing failed", "err", err)
					merr = err
					continue
				}
		...
		if err != nil {
			db.logger.Error("Failed to read database write delay statistic", "err", err)
			merr = err
			continue
		}
		var (
		...
		if err != nil {
			db.logger.Error("Failed to parse delay duration", "err", err)
			merr = err
			continue
		}
		...
		if len(parts) < 2 {
			db.logger.Error("Bad syntax of ioStats", "ioStats", ioStats)
			merr = fmt.Errorf("bad syntax of ioStats %s", ioStats)
			continue
		}
		if n, err := fmt.Sscanf(parts[0], "Read(MB):%f", &nRead); n != 1 || err != nil {
			db.logger.Error("Bad syntax of read entry", "entry", parts[0])
			merr = err
			continue
		}
		if n, err := fmt.Sscanf(parts[1], "Write(MB):%f", &nWrite); n != 1 || err != nil {
			db.logger.Error("Bad syntax of write entry", "entry", parts[1])
			merr = err
			continue
		}
