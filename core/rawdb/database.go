...
func InspectDatabase(db ethdb.Database, keyPrefix, keyStart []byte, logger log.Logger) error {
	...
	for it.Next() {
		...
		if count%1000 == 0 && time.Since(logged) > 8*time.Second {
			logger.Info("Inspecting database", "count", count, "elapsed", common.PrettyDuration(time.Since(start)))
			logged = time.Now()
		}
	}
	...
	if unaccounted.size > 0 {
		logger.Error("Database contains unaccounted data", "size", unaccounted.size, "count", unaccounted.count)
	}
