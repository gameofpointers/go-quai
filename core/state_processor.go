...
func RedeemLockedQuai(hc *HeaderChain, header *types.WorkObject, parent *types.WorkObject, statedb *state.StateDB) (error, []common.Unlock) {
	...
	for i, blockDepth := range blockDepths {
		...
		if i == 0 {
			if currentBlockHeight >= params.BigSporkForkNumber+params.OldConversionLockPeriod &&
				currentBlockHeight < params.BigSporkForkNumber+params.NewConversionLockPeriod {
				continue
			}

			if currentBlockHeight >= params.BigSporkForkNumber+params.NewConversionLockPeriod {
		...
		for _, etx := range targetBlock.Body().ExternalTransactions() {
			...
			if types.IsConversionTx(etx) && etx.To().IsInQuaiLedgerScope() && i == 0 {
				internal, err := etx.To().InternalAddress()
				if err != nil {
