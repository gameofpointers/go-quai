					...
					for denomination := types.MaxDenomination; denomination >= 0; denomination-- {
						...
						for j := uint64(0); j < denominations[uint8(denomination)]; j++ {
							...
							*usedGas += params.CallValueTransferGas    // In the future we may want to determine what a fair gas cost is
							...
							lock := big.NewInt(0)
							if block.PrimeTerminusNumber().Uint64() >= params.OrchardConversionRefundKickBlock && etx.EtxType() == types.UnwrapQiType {
								lock = new(big.Int).Add(big.NewInt(int64(params.ConversionLockPeriod)), block.Number(common.ZONE_CTX))
							}
...
func ValidateQiTxInputs(tx *types.Transaction, chain ChainContext, db ethdb.Reader, currentHeader *types.WorkObject, signer types.Signer, location common.Location, chainId big.Int) (*big.Int, error) {
	...
	if len(tx.Data()) != 0 && len(tx.Data()) != common.AddressLength && currentHeader.NumberU64(common.ZONE_CTX) < params.OrchardBaseFeeChangeBlock {
		return nil, fmt.Errorf("tx %v emits UTXO with invalid data length %d", tx.Hash().Hex(), len(tx.Data()))
	}

	if currentHeader.NumberU64(common.ZONE_CTX) >= params.OrchardBaseFeeChangeBlock {
...
func ValidateQiTxOutputsAndSignature(tx *types.Transaction, chain ChainContext, totalQitIn *big.Int, currentHeader *types.WorkObject, signer types.Signer, location common.Location, chainId big.Int, qiScalingFactor float64, etxRLimit, etxPLimit uint64) (*big.Int, error) {
	...
	for txOutIdx, txOut := range tx.TxOut() {
		...
		if (toAddr.Location().Equal(location) && toAddr.IsInQuaiLedgerScope() && len(tx.Data()) == 0 && currentHeader.PrimeTerminusNumber().Uint64() < params.OrchardConversionRefundKickBlock) || // Qi->Quai conversion
			(toAddr.Location().Equal(location) && toAddr.IsInQuaiLedgerScope() && len(tx.Data()) == params.MaxQiTxDataLength && currentHeader.PrimeTerminusNumber().Uint64() >= params.OrchardConversionRefundKickBlock) { // Qi->Quai conversion
			conversion = true
			...
			continue
		} else if (toAddr.Location().Equal(location) && toAddr.IsInQuaiLedgerScope() && len(tx.Data()) != 0 && currentHeader.PrimeTerminusNumber().Uint64() < params.OrchardConversionRefundKickBlock) || // Qi wrapping
			(toAddr.Location().Equal(location) && toAddr.IsInQuaiLedgerScope() && len(tx.Data()) == common.AddressLength && currentHeader.PrimeTerminusNumber().Uint64() >= params.OrchardConversionRefundKickBlock) { // Qi wrapping
...
func ProcessQiTx(tx *types.Transaction, chain ChainContext, checkSig bool, isFirstQiTx bool, currentHeader *types.WorkObject, batch ethdb.Batch, db ethdb.Reader, gp *types.GasPool, usedGas *uint64, signer types.Signer, location common.Location, chainId big.Int, qiScalingFactor float64, etxRLimit, etxPLimit *uint64, utxosCreatedDeleted *UtxosCreatedDeleted, supplyAddedQi, supplyRemovedQi *big.Int) (*big.Int, []*types.ExternalTx, *types.Receipt, error, map[string]time.Duration) {
	...
	if len(tx.Data()) != 0 && len(tx.Data()) != common.AddressLength && currentHeader.PrimeTerminusNumber().Uint64() < params.OrchardConversionRefundKickBlock {
		return nil, nil, nil, fmt.Errorf("tx %v emits UTXO with invalid data length %d", tx.Hash().Hex(), len(tx.Data())), nil
	}

	if currentHeader.PrimeTerminusNumber().Uint64() >= params.OrchardConversionRefundKickBlock {
		if len(tx.Data()) != 0 && (len(tx.Data()) != params.MaxQiTxDataLength && len(tx.Data()) != common.AddressLength) {
