	...
	if nodeCtx != common.ZONE_CTX {
		...
		if sl.subClients[location.SubIndex()] != nil {
			...
			if !pEtxs.IsValid(trie.NewStackTrie(nil)) {
				return nil, false, errors.New("sub pending ETXs faild validation")
			}
...
func (sl *Slice) SubRelayPendingHeader(pendingHeader types.PendingHeader, s *big.Int, location common.Location, originCtx int) {
	sl.phCachemu.Lock()
	defer sl.phCachemu.Unlock()
	nodeCtx := common.NodeLocation.Context()
...
// copy big.Int array
func copyBig(original []*big.Int) []*big.Int {
...
func (sl *Slice) writeToPhCacheAndPickPhHead(inSlice bool, reorg bool, s *big.Int, pendingHeaderWithTermini types.PendingHeader, local bool, terminiIndex int, originCtx int) *big.Int {
	...
	if inSlice {
		if reorg {
			...
			if nodeCtx == common.ZONE_CTX {
				newEntropy := copyBig(sl.bestPhKey.entropy)
...
func (sl *Slice) init(genesis *Genesis) error {
	...
	if sl.hc.Empty() {
		...
		sl.bestPhKey = bestPhKeyStruct{key: genesisHash, entropy: []*big.Int{big.NewInt(0), big.NewInt(0), big.NewInt(0)}, blockHash: genesisHash}
		...
		for index := 0; index < common.HierarchyDepth; index++ {
			...
			pendingHeader.SetEntropyThreshold(genesisHeader.EntropyThreshold(index), index)
		}
...
func (sl *Slice) loadLastState() error {
	...
	sl.bestPhKey.entropy = []*big.Int{big.NewInt(0), big.NewInt(0), big.NewInt(0)}
