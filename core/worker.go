			...
			if tx.To().IsInQiLedgerScope() {
				...
				for denomination := types.MaxDenomination; denomination >= 0; denomination-- {
					...
					for j := uint64(0); j < denominations[uint8(denomination)]; j++ {
						...
						lock := big.NewInt(0)
						if env.wo.PrimeTerminusNumber().Uint64() >= params.OrchardConversionRefundKickBlock && tx.EtxType() == types.UnwrapQiType {
							lock = new(big.Int).Add(big.NewInt(int64(params.ConversionLockPeriod)), env.wo.Number(common.ZONE_CTX))
						}
...
func (w *worker) prepareWork(genParams *generateParams, wo *types.WorkObject) (*environment, error) {
	...
	if nodeCtx == common.PRIME_CTX {
		...
		if newWo.NumberU64(common.PRIME_CTX) <= params.ControllerKickInBlock {
			...
			if parent.NumberU64(common.PRIME_CTX) < params.OrchardConversionRefundKickBlock {
...
func (w *worker) processQiTx(tx *types.Transaction, env *environment, primeTerminus *types.WorkObject, parent *types.WorkObject, firstQiTx bool) error {
	...
	if len(tx.Data()) != 0 && len(tx.Data()) != common.AddressLength && env.wo.PrimeTerminusNumber().Uint64() < params.OrchardConversionRefundKickBlock {
		return fmt.Errorf("tx %v emits UTXO with invalid data length %d", tx.Hash().Hex(), len(tx.Data()))
	}

	if env.wo.PrimeTerminusNumber().Uint64() >= params.OrchardConversionRefundKickBlock {
	...
	for txOutIdx, txOut := range tx.TxOut() {
		...
		if (toAddr.Location().Equal(location) && toAddr.IsInQuaiLedgerScope() && len(tx.Data()) == 0 && env.wo.PrimeTerminusNumber().Uint64() < params.OrchardConversionRefundKickBlock) || // Qi->Quai conversion
			(toAddr.Location().Equal(location) && toAddr.IsInQuaiLedgerScope() && len(tx.Data()) == params.MaxQiTxDataLength && env.wo.PrimeTerminusNumber().Uint64() >= params.OrchardConversionRefundKickBlock) { // Qi->Quai conversion
