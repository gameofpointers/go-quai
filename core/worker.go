...
func (w *worker) GeneratePendingHeader(block *types.WorkObject, fill bool, stopChan chan struct{}) (*types.WorkObject, error) {
	...
	default:
		...
		if w.hc.NodeCtx() == common.ZONE_CTX && w.primaryCoinbase.Equal(common.Address{}) && w.hc.ProcessingState() {
			w.logger.Error("Refusing to mine without primaryCoinbase")
		...
		if nodeCtx == common.ZONE_CTX && w.hc.ProcessingState() {
			...
			if primaryCoinbase.IsInQiLedgerScope() {
				work.etxs = append(work.etxs, types.NewTx(&types.ExternalTx{}))
			} else if primaryCoinbase.IsInQuaiLedgerScope() {
				work.etxs = append(work.etxs, types.NewTx(&types.ExternalTx{}))
			}
			...
			// TODO: fee from Quai tx goes to Quai coinbase and fee from Qi tx goes to Qi coinbase
			if primaryCoinbase.IsInQiLedgerScope() {
				...
				coinbaseEtx := types.NewTx(&types.ExternalTx{To: &primaryCoinbase, Value: blockReward, IsCoinbase: true, OriginatingTxHash: origin, ETXIndex: 0, Sender: primaryCoinbase})
...
func (w *worker) prepareWork(genParams *generateParams, wo *types.WorkObject) (*environment, error) {
	...
	if nodeCtx == common.ZONE_CTX && w.hc.ProcessingState() {
		...
		proposedWoHeader := types.NewWorkObjectHeader(newWo.Hash(), newWo.ParentHash(nodeCtx), newWo.Number(nodeCtx), newWo.Difficulty(), newWo.WorkObjectHeader().PrimeTerminusNumber(), newWo.TxHash(), newWo.Nonce(), newWo.Time(), newWo.Location(), newWo.PrimaryCoinbase())
		...
		proposedWoHeader := types.NewWorkObjectHeader(newWo.Hash(), newWo.ParentHash(nodeCtx), newWo.Number(nodeCtx), newWo.Difficulty(), newWo.WorkObjectHeader().PrimeTerminusNumber(), types.EmptyRootHash, newWo.Nonce(), newWo.Time(), newWo.Location(), newWo.PrimaryCoinbase())
		proposedWoBody := types.NewWoBody(newWo.Header(), nil, nil, nil, nil, nil)
		proposedWo := types.NewWorkObject(proposedWoHeader, proposedWoBody, nil)
