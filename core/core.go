...
const (
	...
	downloaderHeaderRetryPeriod = 3    // Time (in seconds) before retrying to append future headers
)
...
type Core struct {
	...
	downloaderHeaders *types.TimedCache
...
func NewCore(db ethdb.Database, config *Config, isLocalBlock func(block *types.Header) bool, txConfig *TxPoolConfig, chainConfig *params.ChainConfig, domClientUrl string, subClientUrls []string, engine consensus.Engine, cacheConfig *CacheConfig, vmConfig vm.Config, genesis *Genesis) (*Core, error) {
	...
	downloaderHeaders, _ := types.NewTimedCache(maxFutureHeaders, futureHeaderTtl)
	...
	if len(headers) > 0 {
		...
		for _, head := range headers {
			...
			if err != nil {
				...
				if _, err := c.InsertChain([]*types.Block{block}, true); err == nil {
