...
func (c *Core) GetPendingEtxs(blockHash common.Hash) *types.PendingEtxs {
	...
	return &types.PendingEtxs{header, pendingEtxs}
}

// InsertPendingEtxs inserts a set of pending ETXs to the pending ETX database
func (c *Core) InsertPendingEtxs(p types.PendingEtxs) error {
	return c.sl.AddPendingEtxs(p.Header, p.Etxs)
}
