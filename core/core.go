	...
	for idx, block := range blocks {
		...
		if order == nodeCtx {
			if !c.processingCache.Contains(block.Hash()) {
				...
				c.logger.Info("Already processing block:", "Number:", block.Header().NumberArray(), "Hash:", block.Hash())
			...
			if err == nil {
				...
				if c.sl.CurrentInfo(block.Header()) {
					c.logger.Info("Cannot append yet.", "loc", c.NodeLocation().Name(), "number", block.Header().NumberArray(), "hash", block.Hash(), "err", err)
				} else {
					c.logger.Debug("Cannot append yet.", "loc", c.NodeLocation().Name(), "number", block.Header().NumberArray(), "hash", block.Hash(), "err", err)
				}
...
func (c *Core) procAppendQueue() {
	...
	if len(hashNumberPriorityList) > 0 {
		c.logger.Info("Size of hashNumberPriorityList", "len", len(hashNumberPriorityList), "first entry", hashNumberPriorityList[0].Number, "last entry", hashNumberPriorityList[len(hashNumberPriorityList)-1].Number)
	}
	...
	if len(c.appendQueue.Keys()) < c_appendQueueThreshold || c.procCounter%int(normalListProcCounter) == 0 {
		...
		if len(hashNumberList) > 0 {
			c.logger.Info("Size of hashNumberList", "len", len(hashNumberList), "first entry", hashNumberList[0].Number, "last entry", hashNumberList[len(hashNumberList)-1].Number)
		}
	}
	c.procCounter++
}
	...
	for i, hashAndNumber := range hashNumberList {
		...
		if block != nil {
			...
			if parentBlock != nil {
				...
				if err != nil {
					c.logger.Warn("Error calculating the parent block order in serviceBlocks", "Hash", parentBlock.Hash(), "Number", parentBlock.Header().NumberArray())
					continue
				}
				nodeCtx := c.NodeLocation().Context()
				if parentHeaderOrder < nodeCtx && c.GetHeaderByHash(parentBlock.Hash()) == nil {
					c.logger.Info("Requesting the dom to get the block if it doesnt have and try to append", "Hash", parentBlock.Hash(), "Order", parentHeaderOrder)
			...
			c.logger.Warn("Entry in the FH cache without being in the db: ", "Hash: ", hashAndNumber.Hash)
		}
	}
}
...
func (c *Core) RequestDomToAppendOrFetch(hash common.Hash, entropy *big.Int, order int) {
	...
	if nodeCtx == common.PRIME_CTX {
		...
		if !exists {
			c.logger.Debug("Block sub asked doesnt exist in append queue, so request the peers for it", "Hash", hash, "Order", order)
...
func (c *Core) printStats() {
	c.logger.Info("Blocks waiting to be appended", "loc", c.NodeLocation().Name(), "len(appendQueue)", len(c.appendQueue.Keys()))
	...
	for _, hash := range c.appendQueue.Keys()[:math.Min(len(c.appendQueue.Keys()), c_appendQueuePrintSize)] {
		if value, exist := c.appendQueue.Peek(hash); exist {
			hashNumber := types.HashAndNumber{Hash: hash.(common.Hash), Number: value.(blockNumberAndRetryCounter).number}
			c.logger.Debug("AppendQueue entry. Number: " + strconv.FormatUint(hashNumber.Number, 10) + ". Hash: " + hashNumber.Hash.String())
		}
	}
