...
const (
	...
	c_maxFutureBlocks                   uint64 = 6       // Number of blocks ahead of the current block to be put in the hashNumberList
	c_maxFutureBlocksRegion             uint64 = 6
...
func (c *Core) procAppendQueue() {
	...
	for _, hash := range c.appendQueue.Keys() {
		if value, exist := c.appendQueue.Peek(hash); exist {
			...
			if c.CurrentHeader().Hash() == c.sl.config.GenesisHash || hashNumber.Number < c.CurrentHeader().NumberU64()+maxFutureBlocks {
