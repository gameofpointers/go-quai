...
func TestETX(t *testing.T) {
	...
	toAddress := common.HexToAddress("0x010978987B569072744dc9426E76590eb6fCfE8B", location)
	toPrivKey, err := crypto.ToECDSA(common.FromHex("0xd30d7f719600ec8e06f6552c6716696e521a743fbd6772fa96ed40080219b2e0"))
	if err != nil {
		t.Fatalf("Failed to convert private key to ECDSA: %v", err)
	}
	...
	inner_tx := types.InternalToExternalTx{ChainID: PARAMS.ChainID, Nonce: nonce, GasTipCap: MINERTIP, GasFeeCap: BASEFEE, ETXGasPrice: big.NewInt(3 * params.GWei), ETXGasLimit: 21000, ETXGasTip: big.NewInt(3 * params.GWei), Gas: GAS * 3, To: &to, Value: VALUE, Data: nil, AccessList: types.AccessList{}}
