...
run-sim:
ifeq (,$(wildcard simulations))
	mkdir simulations
endif
	@nohup ./sim.sh >> simulations/simulations.txt &

stop-sim:
ifeq ($(shell uname -s), $(filter $(shell uname -s), Darwin Linux))
	@-pkill -f ./sim.sh;
	@while pgrep sim.sh >/dev/null; do \
		echo "Stopping the simulation."; \
		sleep 3; \
	done;
else
	@echo "Stopping all currently running simulations.";
	@if pgrep sim.sh; then killall -w sim.sh; fi
endif
