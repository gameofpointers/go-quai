...
// return spaces
func CalculateKQuai(parentExchangeRate *big.Int, minerDifficulty *big.Int, beta0Overbeta1 *big.Int) *big.Int {
	...
	// k_quai = (k_quai * d1 + k_quai * alpha * (-b0 * log(d1) - d1))/d1

	// To keep the maximum amount of precision,
	// denum = d1 * 1/alpha
	// adder = k_quai * denum
	// num = (adder + k_quai * (-b0 * log(d1) - d1))
	...
	denum := new(big.Int).Mul(common.Big2e64, minerDifficulty)
	denum = new(big.Int).Mul(denum, params.OneOverAlpha)
	adder := new(big.Int).Mul(kQuai, denum)

	// Multiply beta0 and d2
	num := new(big.Int).Mul(beta0Overbeta1, d2)
	// Negate num
