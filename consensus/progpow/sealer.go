...
func (progpow *Progpow) Seal(header *types.Header, results chan<- *types.Header, stop <-chan struct{}) error {
	...
	if progpow.config.PowMode == ModeFake || progpow.config.PowMode == ModeFullFake {
		...
		case results <- header:
		default:
			progpow.logger.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", header.SealHash())
		}
		return nil
	}
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				progpow.logger.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", header.SealHash())
			}
			close(abort)
		case <-progpow.update:
...
func (s *remoteSealer) sendNotification(ctx context.Context, url string, json []byte, work [4]string) {
	...
	if err != nil {
		s.progpow.logger.Warn("Can't create remote miner notification", "err", err)
		return
	}
	...
	if err != nil {
		...
		s.progpow.logger.Trace("Notified remote miner", "miner", url, "hash", work[0], "target", work[2])
		resp.Body.Close()
	}
}
...
func (s *remoteSealer) submitWork(nonce types.BlockNonce, sealhash common.Hash) bool {
	if s.currentHeader == nil {
		s.progpow.logger.Error("Pending work without block", "sealhash", sealhash)
		return false
	}
	nodeCtx := s.progpow.config.NodeLocation.Context()
	...
	if header == nil {
		s.progpow.logger.Warn("Work submitted but none pending", "sealhash", sealhash, "curnumber", s.currentHeader.NumberU64(nodeCtx))
		return false
	}
	...
	s.progpow.logger.Trace("Verified correct proof-of-work", "sealhash", sealhash, "elapsed", common.PrettyDuration(time.Since(start)))
	...
	if solution.NumberU64(nodeCtx)+staleThreshold > s.currentHeader.NumberU64(nodeCtx) {
		...
		case s.results <- solution:
			s.progpow.logger.Debug("Work submitted is acceptable", "number", solution.NumberU64(nodeCtx), "sealhash", sealhash, "hash", solution.Hash())
			return true
		default:
			s.progpow.logger.Warn("Sealing result is not read by miner", "mode", "remote", "sealhash", sealhash)
			return false
		}
	}
	// The submitted block is too old to accept, drop it.
	s.progpow.logger.Warn("Work submitted is too old", "number", solution.NumberU64(nodeCtx), "sealhash", sealhash, "hash", solution.Hash())
	return false
}
