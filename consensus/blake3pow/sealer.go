...
func (blake3pow *Blake3pow) Seal(header *types.Header, results chan<- *types.Header, stop <-chan struct{}) error {
	...
	if blake3pow.config.PowMode == ModeFake || blake3pow.config.PowMode == ModeFullFake {
		...
		case results <- header:
		default:
			blake3pow.logger.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", header.SealHash())
		}
		return nil
	}
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				blake3pow.logger.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", header.SealHash())
			}
			close(abort)
...
func (blake3pow *Blake3pow) mine(header *types.Header, id int, seed uint64, abort chan struct{}, found chan *types.Header) {
	...
	blake3pow.logger.Trace("Started blake3pow search for new nonces", "seed", seed)
search:
	for {
		select {
		case <-abort:
			// Mining terminated, update stats and abort
			blake3pow.logger.Trace("Blake3pow nonce search aborted", "attempts", nonce-seed)
			break search
		...
		default:
			...
			if powBuffer.SetBytes(hash).Cmp(target) <= 0 {
				...
				select {
				case found <- header:
					blake3pow.logger.Trace("Blake3pow nonce found and reported", "attempts", nonce-seed, "nonce", nonce)
				case <-abort:
					blake3pow.logger.Trace("Blake3pow nonce found but discarded", "attempts", nonce-seed, "nonce", nonce)
				}
				break search
			}
			nonce++
...
func (s *remoteSealer) sendNotification(ctx context.Context, url string, json []byte, work [4]string) {
	...
	if err != nil {
		s.blake3pow.logger.Warn("Can't create remote miner notification", "err", err)
		return
	}
	...
	if err != nil {
		...
		s.blake3pow.logger.Trace("Notified remote miner", "miner", url, "hash", work[0], "target", work[2])
		resp.Body.Close()
	}
}
...
func (s *remoteSealer) submitWork(nonce types.BlockNonce, sealhash common.Hash) bool {
	if s.currentHeader == nil {
		s.blake3pow.logger.Error("Pending work without block", "sealhash", sealhash)
		return false
	}
	nodeCtx := s.blake3pow.config.NodeLocation.Context()
	...
	if header == nil {
		s.blake3pow.logger.Warn("Work submitted but none pending", "sealhash", sealhash, "curnumber", s.currentHeader.NumberU64(nodeCtx))
		return false
	}
	...
	s.blake3pow.logger.Trace("Verified correct proof-of-work", "sealhash", sealhash, "elapsed", common.PrettyDuration(time.Since(start)))
	...
	if solution.NumberU64(nodeCtx)+staleThreshold > s.currentHeader.NumberU64(nodeCtx) {
		...
		case s.results <- solution:
			s.blake3pow.logger.Debug("Work submitted is acceptable", "number", solution.NumberU64(nodeCtx), "sealhash", sealhash, "hash", solution.Hash())
			return true
		default:
			s.blake3pow.logger.Warn("Sealing result is not read by miner", "mode", "remote", "sealhash", sealhash)
			return false
		}
	}
	// The submitted block is too old to accept, drop it.
	s.blake3pow.logger.Warn("Work submitted is too old", "number", solution.NumberU64(nodeCtx), "sealhash", sealhash, "hash", solution.Hash())
	return false
}
