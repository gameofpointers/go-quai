...
type ChainHeaderReader interface {
	...
	CalculatePowDiffAndCount(parent *types.WorkObject, header *types.WorkObjectHeader, powId types.PowID) (*big.Int, *big.Int)

	CountWorkSharesByAlgo(wo *types.WorkObject) (int, int, int)

	CalculateShareTarget(wo *types.WorkObject) *big.Int
...
type Engine interface {
	...
	Finalize(chain ChainHeaderReader, batch ethdb.Batch, header *types.WorkObject, state *state.StateDB, setRoots bool, parentUtxoSetSize uint64, utxosCreate, utxosDelete []common.Hash, supplyRemovedQi *big.Int) (*multiset.MultiSet, uint64, []*types.SpentUtxoEntry, error)
