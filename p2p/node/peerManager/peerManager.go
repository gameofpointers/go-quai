...
const (
	...
	// c_minPeersFromDb is the number of peers we want to randomly read from the db
	c_minPeersFromDb = 5
)
...
func (pm *BasicPeerManager) getBestPeers(topic *pubsubManager.Topic) map[p2p.PeerID]struct{} {
	return pm.getPeersHelper(pm.peerDBs[topic.String()][Best], topic, c_minPeersFromDb)
}

func (pm *BasicPeerManager) getResponsivePeers(topic *pubsubManager.Topic) map[p2p.PeerID]struct{} {
	return pm.getPeersHelper(pm.peerDBs[topic.String()][Responsive], topic, c_minPeersFromDb)
}

func (pm *BasicPeerManager) getLastResortPeers(topic *pubsubManager.Topic) map[p2p.PeerID]struct{} {
	return pm.getPeersHelper(pm.peerDBs[topic.String()][LastResort], topic, c_minPeersFromDb)
}
