...
// RequestBlockHashByNumber returns the hash for the given block number
func (p *P2PNode) RequestBlockHashByNumber(location common.Location, number *big.Int, datatype interface{}) chan interface{} {
...
func (p *P2PNode) requestAndWait(peerID peer.ID, location common.Location, data interface{}, datatype interface{}, resultChan chan interface{}) {
	...
	if recvd, err := p.requestFromPeer(peerID, location, data, datatype); err == nil {
		log.Global.Tracef("Received %s from peer %s", data, peerID)
		...
		log.Global.Tracef("Error requesting the data from peer peerId: %s, location: %s, data: %s, datatype: %s", peerID, location, data, datatype)
...
func (p *P2PNode) Request(location common.Location, data interface{}, datatype interface{}) chan interface{} {
	switch requestDataType := data.(type) {
	case common.Hash:
		return p.RequestByHash(location, data.(common.Hash), datatype)
	case *big.Int:
		return p.RequestBlockHashByNumber(location, data.(*big.Int), datatype)
	default:
		log.Global.Error("Unsupported Request Type: %s in libp2p", requestDataType)
	}
	return nil
}
