...
func (f *callFrame) processOutput(output []byte, err error, reverted bool) {
	...
	if unpacked, err := abi.UnpackRevert(output, *f.From.Location()); err == nil {
...
// CaptureEnd is called when EVM exits a scope, even if the scope didn't
// execute any code.
func (t *callTracer) CaptureEnd(depth int, output []byte, gasUsed uint64, tim time.Duration, err error, reverted bool) {
	if depth == 0 {
...
func (t *callTracer) OnTxStart(env *vm.VMContext, tx *types.Transaction, from common.Address) {
	t.gasLimit = tx.Gas()
}
