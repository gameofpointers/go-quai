...
// execute any code.
func (t *flatCallTracer) CaptureEnd(depth int, output []byte, gasUsed uint64, tim time.Duration, err error, reverted bool) {
	if t.interrupt.Load() {
		return
	}
	t.tracer.CaptureEnd(depth, output, gasUsed, tim, err, reverted)
...
func (t *flatCallTracer) OnTxStart(env *vm.VMContext, tx *types.Transaction, from common.Address) {
	if t.interrupt.Load() {
		return
	}
	...
	t.activePrecompiles = vm.ActivePrecompiles(rules, env.StateDB.GetLocation())
}
