...
func New(stack *node.Node, config *quaiconfig.Config, nodeCtx int, logger log.Logger) (*Quai, error) {
	// Ensure configuration values are compatible and sane
	if config.Miner.GasPrice == nil || config.Miner.GasPrice.Cmp(common.Big0) <= 0 {
		logger.Warn("Sanitizing invalid miner gas price", "provided", config.Miner.GasPrice, "updated", quaiconfig.Defaults.Miner.GasPrice)
	...
	logger.Info("Allocated trie memory caches", "clean", common.StorageSize(config.TrieCleanCache)*1024*1024, "dirty", common.StorageSize(config.TrieDirtyCache)*1024*1024)
	...
	logger.Warn("Memory location of chainConfig", "location", &chainConfig)
	...
	logger.Info("Chain Config", config.NodeLocation)
	chainConfig.Location = config.NodeLocation // TODO: See why this is necessary
	logger.Info("Node", "Ctx", nodeCtx, "NodeLocation", config.NodeLocation, "genesis location", config.Genesis.Config.Location, "chain config", chainConfig.Location)
	...
	logger.Info("Initialising Quai protocol", "network", config.NetworkId, "dbversion", dbVer)
	...
	logger.Info("Dom client", "url", quai.config.DomUrl)
	...
	if uncleanShutdowns, discards, err := rawdb.PushUncleanShutdownMarker(chainDb, logger); err != nil {
		logger.Error("Could not update unclean-shutdown-marker list", "error", err)
	} else {
		if discards > 0 {
			logger.Warn("Old unclean shutdowns found", "count", discards)
		}
		for _, tstamp := range uncleanShutdowns {
			t := time.Unix(int64(tstamp), 0)
			logger.Warn("Unclean shutdown detected", "booted", t,
...
func (s *Quai) isLocalBlock(header *types.Header) bool {
	...
	if err != nil {
		s.logger.Warn("Failed to retrieve block author", "number", header.NumberU64(s.core.NodeCtx()), "hash", header.Hash(), "err", err)
		return false
	}
