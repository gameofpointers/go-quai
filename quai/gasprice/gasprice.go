...
func NewOracle(backend OracleBackend, params Config, logger log.Logger) *Oracle {
	...
	if blocks < 1 {
		blocks = 1
		logger.Warn("Sanitizing invalid gasprice oracle sample blocks", "provided", params.Blocks, "updated", blocks)
	}
	percent := params.Percentile
	if percent < 0 {
		percent = 0
		logger.Warn("Sanitizing invalid gasprice oracle sample percentile", "provided", params.Percentile, "updated", percent)
	}
	if percent > 100 {
		percent = 100
		logger.Warn("Sanitizing invalid gasprice oracle sample percentile", "provided", params.Percentile, "updated", percent)
	...
	if ignorePrice == nil || ignorePrice.Int64() <= 0 {
		...
		logger.Info("Gasprice oracle is ignoring threshold set", "threshold", ignorePrice)
	}
	return &Oracle{
		backend:          backend,
