					...
					if powId == types.Progpow {
						...
						case types.Kawpow, types.SHA_BTC, types.SHA_BCH, types.Scrypt:
							...
							if api.backend.NodeCtx() == common.ZONE_CTX && auxTemplate != nil {
								...
								coinbaseTransaction := types.NewAuxPowCoinbaseTx(powId, auxTemplate.Height(), auxTemplate.CoinbaseOut(), pendingHeaderForMining.SealHash(), uint32(auxTemplate.NTimeMask()))
